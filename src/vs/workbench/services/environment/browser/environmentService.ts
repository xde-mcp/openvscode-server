...
export class BrowserWorkbenchEnvironmentService implements IBrowserWorkbenchEnvironmentService {
	...
	get webviewExternalEndpoint(): string {
		const endpoint = this.options.webviewEndpoint
			...
			|| 'https://{{uuid}}.vscode.mads-dev-substrate.gitpod.cloud/{{quality}}/{{commit}}/out/vs/workbench/contrib/webview/browser/pre/';

		const webviewExternalEndpointCommit = this.payload?.get('webviewExternalEndpointCommit');
		return endpoint
			.replace('{{commit}}', webviewExternalEndpointCommit ?? this.productService.commit ?? '61be0b00a9ba118ae9c7ce7ead827539296859cd')
